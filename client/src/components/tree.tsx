...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "absolute left-1.5 h-full w-px group rounded-md bg-muted py-3 duration-300 ease-in-out hover:bg-slate-300 rtl:right-1.5",
        className,
      )}
      {...props}
    />
    ...
    return (
      ...
      >
        ...
        <AccordionPrimitive.Content className="relative h-full overflow-hidden text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
          ...
          >
            {children}
            <X className=" absolute right-0 size-3 text-muted-foreground cursor-pointer"/>
          </AccordionPrimitive.Root>
